const NO_RESTRICTED_IMPORTS_RULES = [
  "error",
  {
    patterns: [
      {
...
const dropIdFromPatterns = (patterns) =>
  patterns.map(({ group, message }) => ({ group, message }));
...
function filter_patterns_for_ids(configuration, ids) {
  return configuration.map((rule) => {
    if (hasPatterns(rule)) {
      return {
        ...rule,
        patterns: dropIdFromPatterns(filterPatternsByIds(rule.patterns, ids)),
      };
    }
    return rule;
  });
...
module.exports = {
  ...
  overrides: [
    {
      ...
      rules: {
        "no-restricted-imports": filter_patterns_for_ids(
          NO_RESTRICTED_IMPORTS_RULES,
          ["DENY_PAGES_IMPORT"]
        ),
      },
    },
  ...
  rules: {
    ...
    "no-unused-vars": "off",
    "no-restricted-imports": strip_ids_from_no_restricted_imports(
      NO_RESTRICTED_IMPORTS_RULES
    ),
