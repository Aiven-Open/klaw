...
class ApacheKafkaAclServiceTest {
    ...
    public void loadAcls1() throws Exception {
        ...
        when(clusterApiUtils.getAdminClient(any(), eq(KafkaSupportedProtocol.PLAINTEXT), anyString())).thenReturn(adminClient);
        ...
        Set<Map<String, String>> result = apacheKafkaAclService.loadAcls("localhost", KafkaSupportedProtocol.PLAINTEXT, "");
    ...
    public void createProducer() throws Exception {
        ClusterAclRequest clusterAclRequest = getAclRequest(AclType.PRODUCER.value, null, AclIPPrincipleType.IP_ADDRESS.name(), RequestOperationType.CREATE);

        when(clusterApiUtils.getAdminClient(anyString(), eq(KafkaSupportedProtocol.PLAINTEXT), anyString())).thenReturn(adminClient);
        mockCreateAclsRequest();
        mockDescribeAclsRequest();
        when(kafkaFutureCollection.get(anyLong(), any(TimeUnit.class))).thenReturn(Collections.emptyList());
    ...
    public void createProducerAcl_AclAlreadyExists(String aclSsl) throws Exception {
        ClusterAclRequest clusterAclRequest = getAclRequest(AclType.PRODUCER.value, aclSsl, AclIPPrincipleType.PRINCIPAL.name(), RequestOperationType.CREATE);

        when(clusterApiUtils.getAdminClient(anyString(), eq(KafkaSupportedProtocol.PLAINTEXT), anyString())).thenReturn(adminClient);
        mockDescribeAclsRequest();
        when(kafkaFutureCollection.get(anyLong(), any(TimeUnit.class))).thenReturn(List.of(mock(AclBinding.class)));
    ...
    public void updateProducerAcl(String aclSsl) throws Exception {
        ClusterAclRequest clusterAclRequest = getAclRequest(AclType.PRODUCER.value, aclSsl, AclIPPrincipleType.PRINCIPAL.name(), RequestOperationType.UPDATE);
    ...
    public void createConsumerAcl(String aclSsl) throws Exception {
        ClusterAclRequest clusterAclRequest = getAclRequest(AclType.CONSUMER.value, aclSsl, AclIPPrincipleType.PRINCIPAL.name(), RequestOperationType.CREATE);
    ...
    public void updateConsumerAcl(String aclSsl) throws Exception {
        ClusterAclRequest clusterAclRequest = getAclRequest(AclType.CONSUMER.value, aclSsl, AclIPPrincipleType.PRINCIPAL.name(), RequestOperationType.UPDATE);
    ...
    private ClusterAclRequest getAclRequest(String aclType, String aclSsl, String principleType, RequestOperationType requestOperationType) {
