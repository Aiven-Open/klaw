...
class MonitoringServiceTest {
    ...
    @Mock
    private ListOffsetsResult listOffsetsEarliestResult;
    ...
    void getConsumerGroupDetails() throws Exception {
        ...
        Map<String, KafkaFuture<TopicDescription>> nameTopicDescriptionFutures = Map.of("topicName", KafkaFuture.completedFuture(topicDescription));
        TopicPartition topicPartition = new TopicPartition(topicName, partition);
        KafkaFuture<ListOffsetsResult.ListOffsetsResultInfo> listOffsetResultInfoFutures =
                KafkaFuture.completedFuture(new ListOffsetsResult.ListOffsetsResultInfo(offset, 1, Optional.of(1)));
        ...
        Mockito.when(clusterApiUtils.getAdminClient(environment, protocol, clusterName)).thenReturn(adminClient);
        Mockito.when(adminClient.describeTopics(Collections.singletonList(topicName))).thenReturn(describeTopicsResult);
        ...
        Mockito.when(listOffsetsEarliestResult.partitionResult(any())).thenReturn(listOffsetResultInfoFutures);

        List<OffsetDetails> actual = monitoringService.getConsumerGroupDetails(consumerGroupId, topicName, environment,protocol, clusterName);
