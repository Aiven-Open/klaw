...
import com.kafkamgt.uiapi.model.AclType;
...
public class SelectDataJdbc {
    ...
    public List<AclRequests> selectAclRequests(boolean allReqs, String requestor, String role, String status,
        ...
        for (AclRequests row : aclListSub) {
            ...
            if(allReqs) {
                if(role.equals("requestor_subscriptions"))
    ...
    public List<KwKafkaConnector> selectConnectorDetails(String connectorName, int tenantId){
        List<KwKafkaConnector> topicOpt =  kafkaConnectorRepo.findAllByConnectorNameAndTenantId(connectorName, tenantId);
    ...
    public List<Topic> selectAllTopicsByTopictypeAndTeamname(String isProducerConsumer, Integer teamId, int tenantId){
    ...
    public List<TopicRequest> selectTopicRequestsByStatus(boolean allReqs, String requestor, String status,
                                                          boolean showRequestsOfAllTeams, int tenantId){
        ...
        if(allReqs) { // approvers
            List<TopicRequest> claimTopicReqs = topicRequestsRepo.findAllByTopictypeAndTenantId(TopicRequestTypes.Claim.name(), tenantId);

            if(status.equals("all")) {
                topicRequestListSub = topicRequestsRepo.findAllByTenantId(tenantId);
                topicRequestListSub = topicRequestListSub.stream()
                        .filter(topicRequest -> !topicRequest.getTopictype().equals(TopicRequestTypes.Claim.name()))
                        .collect(Collectors.toList());

                claimTopicReqs = claimTopicReqs.stream()
                ...
                claimTopicReqs = claimTopicReqs.stream()
                        .filter(claimTopicReq ->
                                claimTopicReq.getDescription().equals("" + teamSelected) && claimTopicReq.getTopicstatus().equals(status))
                        .collect(Collectors.toList());
            }
        ...
        for (TopicRequest row : topicRequestListSub) {
            ...
            else if(teamSelected!=null && (teamSelected.equals(teamName) || ("" + teamSelected).equals(row.getDescription()))) {
    ...
    public List<KafkaConnectorRequest> selectConnectorRequestsByStatus(boolean allReqs, String requestor, String status,
                                                          boolean showRequestsOfAllTeams, int tenantId){
        ...
        if(allReqs) { // approvers
            List<KafkaConnectorRequest> claimTopicReqs = kafkaConnectorRequestsRepo
                    .findAllByConnectortypeAndTenantId(TopicRequestTypes.Claim.name(), tenantId);

            if(status.equals("all")) {
                topicRequestListSub = Lists.newArrayList(kafkaConnectorRequestsRepo.findAllByTenantId(tenantId));
                topicRequestListSub = topicRequestListSub.stream()
                        .filter(topicRequest -> !topicRequest.getConnectortype().equals(TopicRequestTypes.Claim.name()))
                        .collect(Collectors.toList());
                ...
                topicRequestListSub = kafkaConnectorRequestsRepo.findAllByConnectorStatusAndTenantId(status, tenantId);
                topicRequestListSub = topicRequestListSub.stream()
                        .filter(topicRequest -> !topicRequest.getConnectortype().equals(TopicRequestTypes.Claim.name()))
                        .collect(Collectors.toList());
                ...
                claimTopicReqs = claimTopicReqs.stream()
                        .filter(claimTopicReq ->
                                claimTopicReq.getDescription().equals("" + teamSelected) && claimTopicReq.getConnectorStatus()
                                        .equals(status))
                        .collect(Collectors.toList());
            }
    ...
    public KafkaConnectorRequest selectConnectorRequestsForConnector(int connectorId, int tenantId){
        ...
        Optional<KafkaConnectorRequest> topicReq = kafkaConnectorRequestsRepo.findById(kafkaConnectorRequestID);
        return topicReq.orElse(null);
    }
    ...
    public List<ActivityLog> selectActivityLog(String username, String env, boolean allReqs, int tenantId){
        ...
        if(allReqs) {
            ...
            if(env == null || env.equals(""))
                ...
                activityList = activityLogRepo.findAllByEnvAndTeamIdAndTenantId(env, userInfo.getTeamId(), tenantId);
        }
    ...
    public List<Team> selectTeamsOfUsers(String username, int tenantId){
        ...
        for (UserInfo row : userInfoList) {
            ...
            Optional<Team> teamSel = allTeams.stream().filter(a->a.getTeamId().equals(finalTeamId)).findFirst();
    ...
    public List<HashMap<String, String>> selectActivityLogForLastDays(int numberOfDays, String[] envIdList, int tenantId){
        List<HashMap<String, String>> totalActivityLogCount = new ArrayList<>();
        try {
            List<Object[]> activityCount = activityLogRepo.findActivityLogForLastDays(envIdList, tenantId);
            if(activityCount.size()>numberOfDays)
                activityCount = activityCount.subList(0,numberOfDays-1);
            HashMap<String, String> hashMap;
            for (Object[] actvty : activityCount) {
    ...
    public List<HashMap<String, String>> selectAllMetrics(String metricsType, String metricsName, String env) {
        ...
        try {
            List<Object[]> metrics = kwMetricsRepo.findAllByEnvAndMetricsTypeAndMetricsName(env, metricsType, metricsName);
    ...
    public List<KwClusters> getAllClusters(String typeOfCluster, int tenantId) {
        if(typeOfCluster.equals("all"))
            ...
            return Lists.newArrayList(kwClusterRepo.findAllByClusterTypeAndTenantId(typeOfCluster, tenantId));
    }
    ...
    public List<TopicRequest> selectTopicRequests(String topicName, String envId, String status, int tenantId) {
        return topicRequestsRepo.findAllByTopicstatusAndTopicnameAndEnvironmentAndTenantId(status, topicName, envId, tenantId);
    }

    public List<KafkaConnectorRequest> selectConnectorRequests(String connectorName, String envId, String status, int tenantId) {
        return kafkaConnectorRequestsRepo.findAllByConnectorStatusAndConnectorNameAndEnvironmentAndTenantId(status,
                connectorName, envId, tenantId);
    }

    public String getRegistrationId(String userId) {
        List<RegisterUserInfo> registerInfoList = registerInfoRepo.findAllByUsernameAndStatus(userId, "STAGING");
        List<RegisterUserInfo> registerInfoList1 = registerInfoRepo.findAllByUsernameAndStatus(userId, "PENDING");
    ...
    public RegisterUserInfo getRegistrationDetails(String registrationId, String status) {
        ...
        if(status.equals(""))
            ...
            registerUserInfoList = registerInfoRepo.findAllByRegistrationIdAndStatus(registrationId, status);
    ...
    public int findAllKafkaComponentsCountForEnv(String env, int tenantId) {
        return ((BigInteger)topicRepo.findAllTopicsCountForEnv(env, tenantId).get(0)[0]).intValue() +
                ((BigInteger)topicRequestsRepo.findAllTopicRequestsCountForEnv(env, tenantId).get(0)[0]).intValue() +
                ((BigInteger)aclRepo.findAllAclsCountForEnv(env, tenantId).get(0)[0]).intValue() +
                ((BigInteger)aclRequestsRepo.findAllAclRequestsCountForEnv(env, tenantId).get(0)[0]).intValue();
    }

    public int findAllConnectorComponentsCountForEnv(String env, int tenantId) {
        return ((BigInteger)kafkaConnectorRepo.findAllConnectorCountForEnv(env, tenantId).get(0)[0]).intValue() +
                ((BigInteger)kafkaConnectorRequestsRepo.findAllConnectorRequestsCountForEnv(env, tenantId).get(0)[0]).intValue();
    }

    public int findAllSchemaComponentsCountForEnv(String env, int tenantId) {
        return ((BigInteger)schemaRequestRepo.findAllSchemaRequestsCountForEnv(env, tenantId).get(0)[0]).intValue() +
                ((BigInteger)messageSchemaRepo.findAllSchemaCountForEnv(env, tenantId).get(0)[0]).intValue();
    }

    public int findAllComponentsCountForTeam(Integer teamId, int tenantId) {
        return ((BigInteger)schemaRequestRepo.findAllRecordsCountForTeamId(teamId, tenantId).get(0)[0]).intValue() +
                ((BigInteger)messageSchemaRepo.findAllRecordsCountForTeamId(teamId, tenantId).get(0)[0]).intValue() +
                ((BigInteger)kafkaConnectorRepo.findAllRecordsCountForTeamId(teamId, tenantId).get(0)[0]).intValue() +
                ((BigInteger)kafkaConnectorRequestsRepo.findAllRecordsCountForTeamId(teamId, tenantId).get(0)[0]).intValue() +
                ((BigInteger)topicRepo.findAllRecordsCountForTeamId(teamId, tenantId).get(0)[0]).intValue() +
                ((BigInteger)topicRequestsRepo.findAllRecordsCountForTeamId(teamId, tenantId).get(0)[0]).intValue() +
                ((BigInteger)aclRepo.findAllRecordsCountForTeamId(teamId, tenantId).get(0)[0]).intValue() +
                ((BigInteger)aclRequestsRepo.findAllRecordsCountForTeamId(teamId, tenantId).get(0)[0]).intValue() ;
    }
