...
public class InsertDataJdbc {
    ...
    public synchronized HashMap<String, String> insertIntoRequestConnector(KafkaConnectorRequest connectorRequest){
        ...
        activityLog.setActivityTime(new Timestamp(System.currentTimeMillis()));
        activityLog.setTeamId(userInfo.getTeamId());
        activityLog.setDetails(connectorRequest.getConnectorName() + "," + connectorRequest.getConnectortype());
    ...
    public synchronized String insertIntoConnectorSOT(List<KwKafkaConnector> kafkaConnectors, boolean isSyncTopics){

        kafkaConnectors.forEach(connector ->
                {
                    ...
                    if(!connector.isExistingConnector())
                    {
                        ...
                        {
                            connector.setConnectorId(getNextConnectorRequestId("CONNECTOR_ID", connector.getTenantId()));
                        }
                    }
    ...
    public synchronized String insertIntoRequestSchema(SchemaRequest schemaRequest){
        ...
        schemaRequest.setTeamId(jdbcSelectHelper.selectUserInfo(schemaRequest.getUsername()).getTeamId());
    ...
    public String insertIntoTeams(Team team){
        ...
        if(teamRepo.findAllByTenantId(team.getTenantId()).stream().anyMatch(team1->team1.getTeamname().equals(team.getTeamname()))){
    ...
    public String insertIntoRegisterUsers(RegisterUserInfo userInfo) {
        ...
        if(userExists.isPresent()){
            ...
            else if(!userExists.get().getStatus().equals("STAGING") && !userExists.get().getStatus().equals("PENDING"))
    ...
    public Integer getNextSchemaRequestId(String idType, int tenantId) {
        ...
        else if(idType.equals("SCHEMA_ID"))
