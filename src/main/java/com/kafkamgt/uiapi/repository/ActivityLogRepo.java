...
import com.kafkamgt.uiapi.dao.ActivityLogID;
...
public interface ActivityLogRepo extends CrudRepository<ActivityLog, ActivityLogID> {
    ...
    @Query(value ="select date(activitytime), count(*) from kwactivitylog where" +
            " teamid = :teamIdVar and tenantid = :tenantId group by date(activitytime) order by date(activitytime) asc", nativeQuery = true)
    List<Object[]> findActivityLogForTeamId(@Param("teamIdVar") Integer teamIdVar, @Param("tenantId") Integer tenantId);


    @Query(value ="select date(activitytime), count(*) from kwactivitylog where " +
            " env in :envId and tenantid = :tenantId" +
            " group by date(activitytime) order by date(activitytime) asc;", nativeQuery = true)
    List<Object[]> findActivityLogForLastDays(@Param("envId") String[] envId, @Param("tenantId") Integer tenantId);

    @Query(value ="select max(kwreqno) from kwactivitylog where tenantid = :tenantId", nativeQuery = true)
