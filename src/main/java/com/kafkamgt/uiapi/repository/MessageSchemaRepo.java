...
public interface MessageSchemaRepo extends CrudRepository<MessageSchema, MessageSchemaID> {
    ...
    @Query(value ="select count(*) from kwavroschemas where env = :envId and tenantid = :tenantId", nativeQuery = true)
    List<Object[]> findAllSchemaCountForEnv(@Param("envId") String envId, @Param("tenantId") Integer tenantId);

    @Query(value ="select count(*) from kwavroschemas where teamid = :teamId and tenantid = :tenantId", nativeQuery = true)
    List<Object[]> findAllRecordsCountForTeamId(@Param("teamId") Integer teamId, @Param("tenantId") Integer tenantId);

    @Query(value ="select max(avroschemaid) from kwavroschemas where tenantid = :tenantId", nativeQuery = true)
