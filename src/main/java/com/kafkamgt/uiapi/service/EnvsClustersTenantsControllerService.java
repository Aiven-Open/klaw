...
public class EnvsClustersTenantsControllerService {
    ...
    public synchronized EnvModel getEnvDetails(String envSelected, String clusterType) {
        ...
        if(env != null) {
            ...
            envModel.setClusterName(manageDatabase.getClusters(clusterType, tenantId).get(envModel.getClusterId()).getClusterName());
    ...
    private void extractKwEnvParameters(EnvModel env){
        String defPartns = "", defMaxPartns = "", defaultRf = "",maxRf = "", topicPrefix = "",topicSuffix = "";
    ...
    public List<KwClustersModel> getClusters(String typeOfCluster) {
        int tenantId = commonUtilsService.getTenantId(getUserName());
        List<KwClusters> clusters = new ArrayList<>(manageDatabase.getClusters(typeOfCluster, tenantId).values());
        ...
        for (KwClusters cluster : clusters) {
            ...
            if(!commonUtilsService.isNotAuthorizedUser(getPrincipal(), PermissionType.ADD_EDIT_DELETE_CLUSTERS)){
                if(allEnvList.stream().anyMatch(env->env.getClusterId().equals(finalTmpClusterModel.getClusterId())))
                {
    ...
    public List<KwClustersModel> getClustersPaginated(String typeOfCluster, String clusterId, String pageNo, String searchClusterParam) {
        List<KwClustersModel> kwClustersModelList = getClusters("all");

        if(clusterId != null && !clusterId.equals("")){
            kwClustersModelList = kwClustersModelList.stream()
                    .filter(env -> (env.getClusterId() + "").toLowerCase().equals(clusterId))
                    .collect(Collectors.toList());
        }

        if(searchClusterParam != null && !searchClusterParam.equals("")) {
            List<KwClustersModel> envListMap1 = kwClustersModelList.stream()
                    .filter(env -> env.getClusterName().toLowerCase().contains(searchClusterParam.toLowerCase()))
                    .collect(Collectors.toList());
            List<KwClustersModel> envListMap2 = kwClustersModelList.stream()
                    .filter(env -> env.getBootstrapServers().toLowerCase().contains(searchClusterParam.toLowerCase()))
                    .collect(Collectors.toList());
            List<KwClustersModel> envListMap3 = kwClustersModelList.stream()
                    .filter(env -> env.getProtocol().toLowerCase().contains(searchClusterParam.toLowerCase()))
                    .collect(Collectors.toList());
            envListMap1.addAll(envListMap2);
            envListMap1.addAll(envListMap3);

            // remove duplicates
            kwClustersModelList = envListMap1.stream()
                    .collect(Collectors.collectingAndThen(
                            Collectors.toCollection(() -> new TreeSet<>(Comparator.comparing(KwClustersModel:: getClusterId))),
                            ArrayList::new));
        }
        return getClustersModelsPaginated(pageNo, kwClustersModelList);
    }

    private List<KwClustersModel> getClustersModelsPaginated(String pageNo, List<KwClustersModel> envListMap) {
        List<KwClustersModel> envListMapUpdated = new ArrayList<>();

        int totalRecs = envListMap.size();
        int recsPerPage = 10;

        int totalPages = envListMap.size()/recsPerPage + (envListMap.size()%recsPerPage > 0 ? 1 : 0);
    ...
    public List<HashMap<String,String>> getSyncEnvs() {
        ...
        for (EnvModel env : envList) {
            ...
            if(syncCluster.equals(env.getId()))
    ...
    public List<EnvModel> getKafkaEnvs() {
        ...
        envModelList.forEach(envModel ->
                envModel.setTenantName(manageDatabase
                        .getTenantMap()
                        .get(envModel.getTenantId()))
        );

        // set deletable only to authorized, and check for count
        if(!commonUtilsService.isNotAuthorizedUser(getPrincipal(), PermissionType.ADD_EDIT_DELETE_ENVS)){
            envModelList.forEach(envModel -> {
                envModel.setShowDeleteEnv(manageDatabase.getHandleDbRequests().findAllKafkaComponentsCountForEnv(envModel.getId(), tenantId) <= 0);
            });
        }
...
//        if(commonUtilsService.isNotAuthorizedUser(getPrincipal(), PermissionType.VIEW_EDIT_ALL_ENVS_CLUSTERS_TENANTS)){
...
//        }
        ...
        List<EnvModel> envModelList = getEnvModels(listEnvs, KafkaClustersType.kafkaconnect.value, tenantId);
    ...
    public HashMap<String, List<EnvModel>> getEnvsStatus() {
        Integer tenantId = getUserDetails(getUserName()).getTenantId();
//        Integer tenantId = getTeamDetails(getUserDetails(getUserDetails()).getTeam()).getTenantId();
        HashMap<Integer, List<EnvModel>> allTenantsEnvModels = manageDatabase.getEnvModelsClustersStatusAllTenants();
        HashMap<String, List<EnvModel>> allTenantsEnvModelsUpdated = new HashMap<>();

        if(commonUtilsService.isNotAuthorizedUser(getPrincipal(), PermissionType.ADD_EDIT_DELETE_ENVS)){
            allTenantsEnvModelsUpdated.put(manageDatabase.getTenantMap().get(tenantId), allTenantsEnvModels.get(tenantId));
        }
        else
        {
            for (Integer tenantIdInt : allTenantsEnvModels.keySet()) {
                allTenantsEnvModelsUpdated.put(manageDatabase.getTenantMap().get(tenantIdInt), allTenantsEnvModels.get(tenantIdInt));
            }
        }
    ...
    public List<EnvModel> getEnvsPaginated(String envId, String pageNo, String searchEnvParam){
        ...
        if(envId != null && !envId.equals("")){
            envListMap = envListMap.stream()
                    .filter(env -> env.getId().equals(envId))
                    .collect(Collectors.toList());
        }

        if(searchEnvParam != null && !searchEnvParam.equals("")) {
            List<EnvModel> envListMap1 = envListMap.stream()
                    .filter(env -> env.getName().toLowerCase().contains(searchEnvParam.toLowerCase()))
                    .collect(Collectors.toList());
            List<EnvModel> envListMap2 = envListMap.stream()
                    .filter(env -> env.getClusterName().toLowerCase().contains(searchEnvParam.toLowerCase()))
                    .collect(Collectors.toList());
            List<EnvModel> envListMap3 = envListMap.stream()
                    .filter(env -> env.getOtherParams().toLowerCase().contains(searchEnvParam.toLowerCase()))
                    .collect(Collectors.toList());
            List<EnvModel> envListMap4 = envListMap.stream()
                    .filter(env -> manageDatabase.getTenantMap().get(env.getTenantId()).toLowerCase().contains(searchEnvParam.toLowerCase()))
                    .collect(Collectors.toList());
            envListMap1.addAll(envListMap2);
            envListMap1.addAll(envListMap3);
            ...
            // remove duplicates
            envListMap = envListMap1.stream()
                    .collect(Collectors.collectingAndThen(
                            Collectors.toCollection(() -> new TreeSet<>(Comparator.comparing(EnvModel:: getId))),
                            ArrayList::new));
        }
    ...
    public HashMap<String, List<String>> getEnvParams(String targetEnv) {
        return manageDatabase.getEnvParamsMap(commonUtilsService.getTenantId(getUserName())).get(targetEnv);
    }
    ...
    public List<EnvModel> getSchemaRegEnvs() {
        ...
        List<EnvModel> envModelList = getEnvModels(listEnvs, KafkaClustersType.schemaregistry.value, tenantId);
        ...
        if(!commonUtilsService.isNotAuthorizedUser(getPrincipal(), PermissionType.ADD_EDIT_DELETE_ENVS)){
            envModelList.forEach(envModel -> {
                envModel.setShowDeleteEnv(manageDatabase.getHandleDbRequests().findAllConnectorComponentsCountForEnv(envModel.getId(), tenantId) <= 0);
            });
        }
    ...
    public List<EnvModel> getSchemaRegEnvsStatus() {
        List<Env> listEnvs = manageDatabase.getHandleDbRequests().selectAllSchemaRegEnvs(commonUtilsService.getTenantId(getUserName()));
        int tenantId = commonUtilsService.getTenantId(getUserName());
        List<String> allowedEnvIdList = getEnvsFromUserId();
        listEnvs = listEnvs.stream()
                .filter(env -> allowedEnvIdList.contains(env.getId()))
        ...
        for(Env oneEnv: listEnvs){
           ...
           if (manageDatabase.getClusters(KafkaClustersType.schemaregistry.value, tenantId)
                   .get(oneEnv.getClusterId()).getProtocol().equalsIgnoreCase("plaintext"))
                status = clusterApiService.getSchemaClusterStatus(manageDatabase.getClusters(KafkaClustersType.schemaregistry.value, tenantId)
        ...
        return getEnvModels(newListEnvs, KafkaClustersType.schemaregistry.value, tenantId);
    }
    ...
    public String addNewEnv(EnvModel newEnv){
        ...
        if(newEnv.getId() == null || newEnv.getId().length()==0) {
            ...
            boolean envNameAlreadyPresent = envActualList.stream()
                    .anyMatch(en->en.getName().equals(newEnv.getName()) && en.getType().equals(newEnv.getType()) &&
                            en.getTenantId().equals(newEnv.getTenantId()) &&
                            en.getEnvExists().equals("true")); // 504 change
            if(envNameAlreadyPresent){
                return "{\"result\":\"Failure "+ "Please choose a different name. This environment name already exists." +"\"}";
            }
            else if(envActualList.stream()
                    .anyMatch(en->en.getName().equals(newEnv.getName()) && en.getType().equals(newEnv.getType()) &&
                            en.getTenantId().equals(newEnv.getTenantId()) &&
                            en.getEnvExists().equals("false"))){
                Optional<Env> envAlreadyExistsInDeleted = envActualList.stream()
                        .filter(en->en.getName().equals(newEnv.getName()) && en.getType().equals(newEnv.getType()) &&
                                en.getTenantId().equals(newEnv.getTenantId()) &&
                                en.getEnvExists().equals("false")).findFirst();
        ...
        if(!envIdAlreadyExistsInDeleteStatus.equals(""))
    ...
    public HashMap<String, String> addNewCluster(KwClustersModel kwClustersModel){
        ...
        if(kwClustersModel.getClusterId() == null) {
            manageDatabase.getClusters("all", tenantId)
                .forEach((k,v)->
                        {
            ...
            if(clusterNameAlreadyExists.get()){
                resultMap.put("result", "Failure. Please choose a different name. This cluster name already exists.");
                return resultMap;
            }
        }
        ...
        if(kwCluster.getProtocol().equals("SSL") && kwCluster.getClusterId() == null && kwInstallationType.equals("saas")){
    ...
    private boolean savePublicKey(KwClustersModel kwClustersModel, HashMap<String, String> resultMap, int tenantId, KwClusters kwCluster) {
        try {
            kwCluster.setPublicKey(kwClustersModel.getPublicKey());
            boolean pubKeyFileCreated = createPublicKeyFile(kwCluster.getClusterName().toUpperCase(), tenantId, kwClustersModel.getPublicKey());
            if(pubKeyFileCreated){
                return commonUtilsService.addPublicKeyToTrustStore(kwCluster.getClusterName().toUpperCase(), tenantId);
            }
            else return false;
        }catch (Exception e){
            e.printStackTrace();
    ...
    public String deleteCluster(String clusterId) {
        ...
        List<Env> allEnvList = manageDatabase.getAllEnvList(tenantId);
        if(allEnvList.stream().anyMatch(env->env.getClusterId().equals(Integer.parseInt(clusterId))))
        {
            return "{\"result\":\"Not allowed to delete this cluster, as there are associated environments.\"}";
        }

        try {
            String result = manageDatabase.getHandleDbRequests().deleteCluster(Integer.parseInt(clusterId), tenantId);
    ...
    public HashMap<String, String> deleteEnvironment(String envId, String envType) {
        ...
        switch (envType) {
            case "kafka":
                if (manageDatabase.getHandleDbRequests().findAllKafkaComponentsCountForEnv(envId, tenantId) > 0) {
                    String notAllowed = "Not allowed to delete this environment, as there are associated topics/acls/requests.";
                    resultMap.put("result", notAllowed);
                    return resultMap;
                }
                break;
            case "kafkaconnect":
                if (manageDatabase.getHandleDbRequests().findAllConnectorComponentsCountForEnv(envId, tenantId) > 0) {
                    String notAllowed = "Not allowed to delete this environment, as there are associated connectors/requests.";
                    resultMap.put("result", notAllowed);
                    return resultMap;
                }
                break;
            case "schemaregistry":
                if (manageDatabase.getHandleDbRequests().findAllSchemaComponentsCountForEnv(envId, tenantId) > 0) {
                    String notAllowed = "Not allowed to delete this environment, as there are associated schemaregistry/requests.";
                    resultMap.put("result", notAllowed);
                    return resultMap;
                }
                break;
        ...
        try {
            String result = manageDatabase.getHandleDbRequests().deleteEnvironmentRequest(envId, tenantId);
            commonUtilsService.updateMetadata(tenantId, EntityType.ENVIRONMENT, MetadataOperationType.DELETE);
    ...
    public List<KwTenantModel> getAllTenants(){
        if(manageDatabase.getHandleDbRequests().getUsersInfo(getUserName()).getRole().equals(SUPERADMIN.name())
            ...
            for (KwTenants tenant : tenants) {
                ...
                Optional<UserInfo> userFound = allUsers.stream()
                        .filter(userInfo -> userInfo.getTenantId() == tenant.getTenantId()
                                && userInfo.getRole().equals(SUPERADMIN_ROLE)).findFirst();
    ...
    public List<String> getStandardEnvNames() {
        int tenantId = commonUtilsService.getTenantId(getUserName());
        String standardNames = manageDatabase.getKwPropertyValue("kafkawize.envs.standardnames", tenantId);
    ...
    public HashMap<String, String> addTenantId(KwTenantModel kwTenantModel, boolean isExternal) {
        ...
        if(manageDatabase.getHandleDbRequests().getTenants().size() >= maxNumberOfTenantsCanBeCreated){
            addTenantStatus.put("result", "Maximum tenants reached.");
            return addTenantStatus;
        }

        if(isExternal && commonUtilsService.isNotAuthorizedUser(getPrincipal(), PermissionType.ADD_TENANT))
        {
        ...
        if(kwInstallationType.equals("saas"))
            kwTenants.setLicenseExpiry(new Timestamp(System.currentTimeMillis() + TimeUnit.DAYS.toMillis (DAYS_TRIAL_PERIOD)));
        else
        ...
        int tenantId = manageDatabase.getHandleDbRequests().getTenants().stream()
        ...
        if(isExternal){
            manageDatabase.getHandleDbRequests().insertDefaultKwProperties(defaultDataService.createDefaultProperties(tenantId, ""));
            manageDatabase.getHandleDbRequests().insertDefaultRolesPermissions(defaultDataService.createDefaultRolesPermissions(tenantId, false, kwInstallationType));

            commonUtilsService.updateMetadata(tenantId, EntityType.ROLES_PERMISSIONS, MetadataOperationType.CREATE);
            commonUtilsService.updateMetadata(tenantId, EntityType.PROPERTIES, MetadataOperationType.CREATE);
        }
    ...
    public KwTenantModel getMyTenantInfo() {
        ...
        if(tenant.isPresent()){
            ...
            kwTenantModel.setLicenseExpiryDate(((new SimpleDateFormat("dd-MMM-yyyy HH:mm:ss"))
            ...
            long timeInMilliSeconds = tenant.get().getLicenseExpiry().getTime()-System.currentTimeMillis();
            ...
            if(commonUtilsService.isNotAuthorizedUser(getPrincipal(), PermissionType.UPDATE_DELETE_MY_TENANT))
            {
    ...
    public List<String> getExtensionPeriods() {
        ...
        //return Arrays.asList("1 month (7$)", "2 months (14$)", "3 months (20$)", "6 months", "1 year", "2 years", "3 years", "5 years");
    }
    ...
    public HashMap<String,String> udpateTenantExtension(String selectedTenantExtensionPeriod) {
        ...
        String result = mailService.sendMailToSaasAdmin(tenantId, getUserName(), selectedTenantExtensionPeriod,
