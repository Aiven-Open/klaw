...
public class RolesPermissionsControllerService {
    ...
    public List<String> getRoles() {
        ...
        try {
            if(commonUtilsService.isNotAuthorizedUser(getPrincipal(), PermissionType.FULL_ACCESS_USERS_TEAMS_ROLES))
    ...
    public List<String> getRolesFromDb() {
        return new ArrayList<>(manageDatabase.getRolesPermissionsPerTenant(commonUtilsService.getTenantId(getUserName())).keySet());
    }


    public HashMap<String, List<HashMap<String, Boolean>>> getPermissions(boolean isExternalCall) {

        if(isExternalCall && commonUtilsService.isNotAuthorizedUser(getPrincipal(), PermissionType.UPDATE_PERMISSIONS))
        {
            return null;
        }
        ...
        HashMap<String, List<String>> existingPerms = manageDatabase.getRolesPermissionsPerTenant(commonUtilsService.getTenantId(getUserName()));
    ...
    public HashMap<String, String> updatePermissions(KwRolesPermissionsModel[] permissionsSet) {
        ...
        for (KwRolesPermissionsModel kwRolesPermissionsModel : permissionsSet) {
            uniqueMap.put(kwRolesPermissionsModel.getRolePermission(), kwRolesPermissionsModel.getPermissionEnabled());
        }
        ...
        for (String permKey : uniqueMap.keySet()) {
            ...
            if(isPermEnabled.equals("true"))
                kwRolesPermissionsAdd.add(tmpKwRolePermModel);
            else if(isPermEnabled.equals("false"))
    ...
    public HashMap<String,String> deleteRole(String roleId) {
        ...
        String status = manageDatabase.getHandleDbRequests().deleteRole(roleId, commonUtilsService.getTenantId(getUserName()));
    ...
    public HashMap<String,String> addRoleId(String roleId) {
        ...
        String status = manageDatabase.getHandleDbRequests().updatePermissions(kwRolesPermissionsAdd,"ADD");
    ...
    protected List<String> getApproverRoles(String requestType,  int tenantId){
        ...
        for (Map.Entry<String, List<HashMap<String, Boolean>>> permissionsListEntry : existingPermissions.entrySet()) {
            ...
            for (HashMap<String, Boolean> entryDet : entryDets) {
                for (Map.Entry<String, Boolean> stringBooleanEntry : entryDet.entrySet()) {
                    if(stringBooleanEntry.getValue().equals(true)){
                        if(requestType.equals("SUBSCRIPTIONS") && stringBooleanEntry.getKey().equals(PermissionType.APPROVE_SUBSCRIPTIONS.name()))
                        {
                            approverRoles.add(permissionsListEntry.getKey());
                            break;
                        }
                        else if(requestType.equals("TOPICS") && stringBooleanEntry.getKey().equals(PermissionType.APPROVE_TOPICS.name()))
                        {
                            approverRoles.add(permissionsListEntry.getKey());
                            break;
                        }
                        else if(requestType.equals("SCHEMAS") && stringBooleanEntry.getKey().equals(PermissionType.APPROVE_SCHEMAS.name()))
                        {
                            approverRoles.add(permissionsListEntry.getKey());
                            break;
                        }
                        else if(requestType.equals("CONNECTORS") && stringBooleanEntry.getKey().equals(PermissionType.APPROVE_SCHEMAS.name()))
                        {
                            approverRoles.add(permissionsListEntry.getKey());
                            break;
                        }
