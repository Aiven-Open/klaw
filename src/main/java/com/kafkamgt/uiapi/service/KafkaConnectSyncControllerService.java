...
public class KafkaConnectSyncControllerService {
    ...
    public HashMap<String, String> updateSyncConnectors(List<SyncConnectorUpdates> updatedSyncTopics) {
        ...
        int topicId = manageDatabase.getHandleDbRequests().getNextConnectorRequestId("CONNECTOR_ID", tenantId);
        ...
        if(updatedSyncTopics.size() > 0){
            for (SyncConnectorUpdates topicUpdate : updatedSyncTopics) {
                ...
                if(existingTopics != null){
                    ...
                    if(checkInPromotionOrder(topicUpdate.getConnectorName(), topicUpdate.getEnvSelected(), orderOfEnvs))
                ...
                if (existingTopics == null) {
                    ...
                    for (KwKafkaConnector existingTopic : existingTopics)
                    {
                        if (existingTopic.getEnvironment().equals(topicUpdate.getEnvSelected())) {
                            ...
                            kafkaConnectorList.add(t);
                            topicAdded = true;
                        }else if(!existingTopic.getTeamId().equals(manageDatabase.getTeamIdFromTeamName(tenantId, topicUpdate.getTeamSelected()))){
                            t = existingTopic;
                            t.setTeamId(manageDatabase.getTeamIdFromTeamName(tenantId, topicUpdate.getTeamSelected()));
                            t.setConnectorName(topicUpdate.getConnectorName());
                            t.setConnectorConfig(connectorConfig);
        ...
        if(topicsDontExistInMainCluster){
            response.put("result", "Failure. Please sync up the team of the following connector(s) first in" +
        ...
        if(topicsWithDiffTeams) {
            response.put("result", "Failure. The following connectors are being synchronized with" +
        ...
        if(kafkaConnectorList.size()>0){
            response.put("result", manageDatabase.getHandleDbRequests().addToSyncConnectors(kafkaConnectorList));
        }
        else
    ...
    private List<SyncConnectorUpdates> handleConnectorDeletes(List<SyncConnectorUpdates> updatedSyncTopics, List<Integer> updatedSyncTopicsDelete) {
        ...
        for (Integer topicId : updatedSyncTopicsDelete) {
            manageDatabase.getHandleDbRequests().deleteConnector(topicId, commonUtilsService.getTenantId(getUserName()));
        }
    ...
    public List<KafkaConnectorModel> getSyncConnectors(String envId, String pageNo, String currentPage, String connectorNameSearch,
        ...
        // get from metastore
        List<KafkaConnectorModel> kafkaConnectorModelSourceList = getSyncConnectorsList(envId, teamList, tenantId);
        if(connectorNameSearch != null && connectorNameSearch.length() > 0){
            final String topicSearchFilter = connectorNameSearch;
            kafkaConnectorModelSourceList = kafkaConnectorModelSourceList.stream()
        ...
        try {
            ...
            if(connectorNameSearch != null && connectorNameSearch.length() > 0){
                final String topicSearchFilter = connectorNameSearch;
                allConnectors = allConnectors.stream()
            ...
            kafkaConnectorModelClusterList.removeIf(p -> !allSyncConnectors.isEmpty()
                    && allSyncConnectors.contains(p.getConnectorName()));
            ...
            for (KafkaConnectorModel kafkaConnectorModel : kafkaConnectorModelSourceList) {
                if(!allConnectors.contains(kafkaConnectorModel.getConnectorName())){
                    for (KafkaConnectorModel kafkaConnectorModelCluster : kafkaConnectorModelClusterList) {
                        if(kafkaConnectorModelCluster.getConnectorName().equals(kafkaConnectorModel.getConnectorName())) {
...
//                            possibleTeams.add("REMOVE FROM KAFKAWIZE");
            ...
            for (KafkaConnectorModel kafkaConnectorModel : kafkaConnectorModelClusterList) {
                ...
                if(kafkaConnectorModel.getTeamName() == null || kafkaConnectorModel.getTeamName().equals("")) {
            ...
            // pagination
            kafkaConnectorModelClusterList = getConnectorsPaged(kafkaConnectorModelClusterList, pageNo, currentPage);
    ...
    private List<KafkaConnectorModel> getSyncConnectorsList(String envId, List<String> teamList, int tenantId) {
        //         Get Sync connectors
        List<KwKafkaConnector> connectorsFromSOT = manageDatabase.getHandleDbRequests()
                .getSyncConnectors(envId, null, tenantId);
        ...
        for (KwKafkaConnector kwKafkaConnector : connectorsFromSOT) {
            KafkaConnectorModel kafkaConnectorModel = new KafkaConnectorModel();
            kafkaConnectorModel.setEnvironmentName(getKafkaConnectorEnvDetails(kwKafkaConnector.getEnvironment()).getName());
            ...
            kafkaConnectorModel.setTeamName(manageDatabase.getTeamNameFromTeamId(tenantId, kwKafkaConnector.getTeamId()));
            kafkaConnectorModel.setPossibleTeams(teamList);
    ...
    private ArrayList<KafkaConnectorModel> getConnectorsPaged(List<KafkaConnectorModel> origActivityList, String pageNo, String currentPage){
    ...
    public Env getKafkaConnectorEnvDetails(String envId){
        Optional<Env> envFound = manageDatabase.getKafkaConnectEnvList(commonUtilsService.getTenantId(getUserName())).stream().filter(env->env.getId().equals(envId)).findFirst();
