...
import com.kafkamgt.uiapi.model.charts.Options;
import com.kafkamgt.uiapi.model.charts.Title;
...
public class CommonUtilsService {
    ...
    private
    String enableUserAuthorizationFromAD;
    ...
    @Value("${kafkawize.saas.ssl.clusterapi.truststore:./tmp}")
    private String trustStore;

    @Value("${kafkawize.saas.ssl.clusterapi.truststore.pwd:./tmp}")
    private String trustStorePwd;

    @Value("${kafkawize.metadata.kafkamode:false}")
...
//    KafkaProducerConsumerService kafkaProducerConsumerService;
    ...
    private static HttpComponentsClientHttpRequestFactory requestFactory = ClusterApiService.requestFactory;

    @Value("${kafkawize.uiapi.servers:server1,server2}")
    ...
    private RestTemplate getRestTemplate(){
        if(uiApiServers.toLowerCase().startsWith("https"))
    ...
    public boolean isNotAuthorizedUser(Object principal, PermissionType permissionType) {
        try {
            return !manageDatabase.getRolesPermissionsPerTenant(getTenantId(getUserName(principal)))
    ...
    public ChartsJsOverview getChartsJsOverview(List<HashMap<String, String>> activityCountList,
                                                String title, String yaxisCount, String xaxisLabel,
                                                String xAxisLabelConstant, String yAxisLabelConstant, int tenantId) {
        ...
        if (activityCountList != null) {
            for (HashMap<String, String> hashMap : activityCountList) {
                ...
                if(xaxisLabel.equals("teamid"))
                    labels.add(manageDatabase.getTeamNameFromTeamId(tenantId, Integer.parseInt(hashMap.get(xaxisLabel))));
                else
...
//        options.setScales(scales);
    ...
    public void getAllPagesList(String pageNo, String currentPage, int totalPages, List<String> numList) {
        if(currentPage != null && !currentPage.equals("") && !currentPage.equals(pageNo) && Integer.parseInt(pageNo) > 1 && totalPages > 1){
            numList.add("<<");
            numList.add("<");
        }
        else if(currentPage != null && currentPage.equals(pageNo) && Integer.parseInt(pageNo) > 1 && totalPages > 1){
            numList.add("<<");
            numList.add("<");
        }
    ...
    public void updateMetadata(int tenantId, EntityType entityType, MetadataOperationType operationType) {
    ...
    public synchronized void updateMetadata(KwMetadataUpdates kwMetadataUpdates) {
        if (kwMetadataUpdates.getEntityType().equals(EntityType.TEAM.name())) {
            ...
            manageDatabase.loadTenantTeamsForOneTenant(null, kwMetadataUpdates.getTenantId());
        }else if(kwMetadataUpdates.getEntityType().equals(EntityType.ENVIRONMENT.name()) && kwMetadataUpdates.getOperationType().equals(MetadataOperationType.DELETE.name())){
            manageDatabase.loadEnvMapForOneTenant(kwMetadataUpdates.getTenantId());
            manageDatabase.loadEnvsForOneTenant(kwMetadataUpdates.getTenantId());
        }else if (kwMetadataUpdates.getEntityType().equals(EntityType.TENANT.name()) && kwMetadataUpdates.getOperationType().equals(MetadataOperationType.CREATE.name())){
            manageDatabase.updateStaticDataForTenant(kwMetadataUpdates.getTenantId());
        }else if (kwMetadataUpdates.getEntityType().equals(EntityType.TENANT.name()) && kwMetadataUpdates.getOperationType().equals(MetadataOperationType.DELETE.name())){
            manageDatabase.deleteTenant(kwMetadataUpdates.getTenantId());
        }else if (kwMetadataUpdates.getEntityType().equals(EntityType.TENANT.name()) && kwMetadataUpdates.getOperationType().equals(MetadataOperationType.UPDATE.name())){
    ...
    public String getBaseUrl() {
        if(kwContextPath.equals(""))
            ...
            return ServletUriComponentsBuilder.fromCurrentContextPath().build().toUriString() + "/" + kwContextPath;
    }

    public void resetCacheOnOtherServers(KwMetadataUpdates kwMetadataUpdates){
        log.info("invokeResetEndpoints");
        try {
            String tenantName = manageDatabase.getHandleDbRequests().getTenants().stream()
                    .filter(kwTenant -> kwTenant.getTenantId().equals(kwMetadataUpdates.getTenantId()))
                    .findFirst().get().getTenantName();
            ...
            if(uiApiServers != null && uiApiServers.length()>0)
            {
                ...
                for (String server : servers) {
                    ...
                    String uri = basePath + "/resetMemoryCache/" + tenantName + "/" + kwMetadataUpdates.getEntityType()
