...
public class ServerConfigService {
                ...
                if (propertySource instanceof EnumerablePropertySource) {
                    for (String key : ((EnumerablePropertySource) propertySource).getPropertyNames()) {
                        ...
                        if(!checkPropertyExists(listProps,key) && !key.toLowerCase().contains("path")
                                && !key.contains("secretkey") && !key.contains("password") && !key.contains("username")) {
    ...
    public List<HashMap<String, String>> getAllEditableProps() {
        ...
        if(tenantId != DEFAULT_TENANT_ID)
            return listMap.stream().filter(item -> allowConfigForAdmins.contains(item.get("kwkey"))).collect(Collectors.toList());
        else
            return listMap;
    }
            ...
            if(kwKey.equals(TENANT_CONFIG_PROPERTY)){
                ...
                try {
                    ...
                    if(validateTenantConfig(dynamicObj, tenantId)){
                        ...
                        response.put("result", "Failure. Invalid json / incorrect name values. Check tenant and env details.");
                        return response;
                    }
                } catch (IOException e) {
    ...
    private void updateEnvNameValues(TenantConfig dynamicObj, int tenantId){
        if(dynamicObj.getTenantModel() != null) {
            ...
            if (tenant.getBaseSyncEnvironment() != null)
                tenant.setBaseSyncEnvironment(getEnvDetails(tenant.getBaseSyncEnvironment(), tenantId).getName());

            // syncClusterKafkaConnectName update
            if (tenant.getBaseSyncKafkaConnectCluster() != null)
                tenant.setBaseSyncKafkaConnectCluster(getKafkaConnectEnvDetails(tenant.getBaseSyncKafkaConnectCluster(), tenantId).getName());

            // kafka
            if (tenant.getOrderOfTopicPromotionEnvsList() != null) {
                List<String> tmpOrderList = new ArrayList<>();
                tenant.getOrderOfTopicPromotionEnvsList()
                        .forEach(a -> {
            ...
            if (tenant.getOrderOfConnectorsPromotionEnvsList() != null) {
                ...
                tenant.getOrderOfConnectorsPromotionEnvsList()
                        .forEach(a -> {
            ...
            if (tenant.getRequestTopicsEnvironmentsList() != null) {
                ...
                tenant.getRequestTopicsEnvironmentsList()
                        .forEach(a -> {
                            if (getEnvDetails(a, tenantId) != null)
            ...
            if (tenant.getRequestConnectorsEnvironmentsList() != null) {
                ...
                tenant.getRequestConnectorsEnvironmentsList()
                        .forEach(a -> {
                            if (getKafkaConnectEnvDetails(a, tenantId) != null)
    ...
    private void updateEnvIdValues(TenantConfig dynamicObj){
        if(dynamicObj.getTenantModel() != null){
            ...
            if(tenantModel.getBaseSyncEnvironment()!=null)
                tenantModel.setBaseSyncEnvironment(getEnvDetailsFromName(tenantModel.getBaseSyncEnvironment(), getTenantIdFromName(tenantModel.getTenantName())).getId());

            // syncClusterKafkaConnectName update
            if(tenantModel.getBaseSyncKafkaConnectCluster()!=null)
                tenantModel.setBaseSyncKafkaConnectCluster(getKafkaConnectEnvDetailsFromName(tenantModel.getBaseSyncKafkaConnectCluster(), getTenantIdFromName(tenantModel.getTenantName())).getId());

            // kafka
            if(tenantModel.getOrderOfTopicPromotionEnvsList() != null) {
                List<String> tmpOrderList = new ArrayList<>();
                tenantModel.getOrderOfTopicPromotionEnvsList().forEach(a -> tmpOrderList.add(getEnvDetailsFromName(a, getTenantIdFromName(tenantModel.getTenantName())).getId()));
            ...
            if(tenantModel.getOrderOfConnectorsPromotionEnvsList() != null) {
                List<String> tmpOrderList1 = new ArrayList<>();
                tenantModel.getOrderOfConnectorsPromotionEnvsList().forEach(a -> tmpOrderList1.add(getKafkaConnectEnvDetailsFromName(a, getTenantIdFromName(tenantModel.getTenantName())).getId()));
            ...
            if(tenantModel.getRequestTopicsEnvironmentsList() != null) {
                List<String> tmpReqTopicList = new ArrayList<>();
                tenantModel.getRequestTopicsEnvironmentsList().forEach(a -> tmpReqTopicList.add(getEnvDetailsFromName(a, getTenantIdFromName(tenantModel.getTenantName())).getId()));
            ...
            if(tenantModel.getRequestConnectorsEnvironmentsList() != null) {
                List<String> tmpReqTopicList1 = new ArrayList<>();
                tenantModel.getRequestConnectorsEnvironmentsList().forEach(a -> tmpReqTopicList1.add(getKafkaConnectEnvDetailsFromName(a, getTenantIdFromName(tenantModel.getTenantName())).getId()));
    ...
    private boolean validateTenantConfig(TenantConfig dynamicObj, int tenantId) {
        ...
        try {
            ...
            if (tenantCheck) {
                ...
                KwTenantConfigModel tenantModel = dynamicObj.getTenantModel();
                    // syncClusterCheck
                    if (tenantModel.getBaseSyncEnvironment() != null && !envListStr.contains(tenantModel.getBaseSyncEnvironment())) {
                        return false;
                    }

                    // syncClusterKafkaConnectCheck
                    if (tenantModel.getBaseSyncKafkaConnectCluster() != null && !envListKafkaConnectStr.contains(tenantModel.getBaseSyncKafkaConnectCluster())) {
                        return false;
                    }
                    ...
                    if (tenantModel.getOrderOfConnectorsPromotionEnvsList() != null) {
                        for (String orderOfConnectorsPromotionEnv : tenantModel.getOrderOfConnectorsPromotionEnvsList()) {
    ...
    public Env getKafkaConnectEnvDetails(String envId, int tenantId){
        Optional<Env> envFound = manageDatabase.getKafkaConnectEnvList(tenantId).stream().filter(env->env.getId().equals(envId)).findFirst();
        return envFound.orElse(null);
    }
    ...
    public Env getKafkaConnectEnvDetailsFromName(String envName, Integer tenantId){
        Optional<Env> envFound = manageDatabase.getKafkaConnectEnvList(tenantId).stream()
                .filter(env->env.getName().equals(envName) && env.getTenantId().equals(tenantId))
                .findFirst();
        return envFound.orElse(null);
    }
    ...
    public HashMap<String, String> testClusterApiConnection(String clusterApiUrl) {
        ...
        if(clusterApiStatus.equals("ONLINE"))
