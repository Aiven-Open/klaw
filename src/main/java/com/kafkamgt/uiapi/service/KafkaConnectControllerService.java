...
import com.kafkamgt.uiapi.model.connectorconfig.ConnectorConfig;
...
public class KafkaConnectControllerService {
    ...
    public HashMap<String, String> createConnectorRequest(KafkaConnectorRequestModel topicRequestReq) {
        log.info("createConnectorRequest {}", topicRequestReq);
        String userDetails = getUserName();
        
        HashMap<String, String> hashMapTopicReqRes = new HashMap<>();

        if(commonUtilsService.isNotAuthorizedUser(getPrincipal(), PermissionType.REQUEST_CREATE_CONNECTORS)){
        ...
        try {
            if(topicRequestReq.getConnectorConfig() != null && topicRequestReq.getConnectorConfig().trim().length() > 0) {
                ...
                if(!jsonNode.has("tasks.max")){
                    ...
                    hashMapTopicReqRes.put("result", "Failure. Invalid config. connector.class is not configured");
                    return hashMapTopicReqRes;
                }
                ...
                Map<String, Object> resultMap = objectMapper.convertValue(jsonNode, new TypeReference<>(){});
                topicRequestReq.setConnectorConfig(objectMapper.writerWithDefaultPrettyPrinter()
        ...
        {
            hashMapTopicReqRes.put("result", "Failure. Not authorized to request connector for this environment.");
            return hashMapTopicReqRes;
        }
        ...
        List<KwKafkaConnector> kafkaConnectorList = getConnectorsFromName(topicRequestReq.getConnectorName(), tenantId);
        ...
        boolean promotionOrderCheck =  checkInPromotionOrder(topicRequestReq.getConnectorName(),
                topicRequestReq.getEnvironment(), orderOfEnvs);

        if(kafkaConnectorList!=null && kafkaConnectorList.size()>0){
            if(promotionOrderCheck) {
                int devTopicFound = (int) kafkaConnectorList.stream()
                        .filter(topic -> topic.getEnvironment().equals(syncCluster)).count();
                if (devTopicFound != 1) {
                    if(getKafkaConnectEnvDetails(syncCluster) == null){
                        hashMapTopicReqRes.put("result", "Failure. This connector does not exist in base cluster");
                    }else {
                        hashMapTopicReqRes.put("result", "Failure. This connector does not exist in " + getKafkaConnectEnvDetails(syncCluster).getName() + " cluster.");
                    }
                    return hashMapTopicReqRes;
                }
            }
            ...
            if(promotionOrderCheck) {
                hashMapTopicReqRes.put("result", "Failure. Please request for a connector first in " +
                        getKafkaConnectEnvDetails(syncCluster).getName() + " cluster.");
                return hashMapTopicReqRes;
            }
        }

        if(kafkaConnectorList != null){
            if(manageDatabase.getHandleDbRequests().selectConnectorRequests(topicRequestReq.getConnectorName(),
                    topicRequestReq.getEnvironment(),
                    RequestStatus.created.name(), tenantId).size()>0){
        ...
        if(!topicRequestReq.getConnectortype().equals(TopicRequestTypes.Update.name())){
            ...
            if(kafkaConnectorList != null) {
                topicExists = kafkaConnectorList.stream()
                        .anyMatch(topicEx-> topicEx.getEnvironment().equals(topicRequestReq.getEnvironment())
                        );
            }
            if(topicExists){
                hashMapTopicReqRes.put("result", "Failure. This connector already exists in the selected cluster.");
                return hashMapTopicReqRes;
            }
        }
        ...
        mailService.sendMail(topicRequestReq.getConnectorName(), null, "", userDetails, dbHandle,
                CONNECTOR_CREATE_REQUESTED, commonUtilsService.getLoginUrl());

        return hashMapTopicReqRes;
    }

    public List<List<KafkaConnectorModel>> getConnectors(String env, String pageNo, String currentPage,
                                                         String connectorNameSearch,
                                                         String teamName) throws Exception {
        log.debug("getConnectors {}", connectorNameSearch);
        List<KafkaConnectorModel> topicListUpdated = getConnectorsPaginated(env, pageNo, currentPage, connectorNameSearch, teamName);
    ...
    private void updateTeamNamesForDisplay(List<KafkaConnectorModel> topicListUpdated) {
        topicListUpdated.forEach(topicInfo ->
        {
    ...
    private List<KafkaConnectorModel> getConnectorsPaginated(String env, String pageNo, String currentPage, String connectorNameSearch,
                                                             String teamName) {
        ...
        List<KwKafkaConnector> topicsFromSOT = handleDbRequests.getSyncConnectors(env, manageDatabase.getTeamIdFromTeamName(tenantId, teamName), tenantId);
        ...
        if(connectorNameSearch!=null && connectorNameSearch.length()>0){
            ...
            List<KwKafkaConnector> searchDocList = topicsFromSOT.stream()
            ...
            topicFilteredList = new ArrayList<>(topicFilteredList.stream()
                    .collect(Collectors.toConcurrentMap(KwKafkaConnector::getConnectorName, Function.identity(), (p, q) -> p))
                    .values());
        }

        topicsFromSOT = topicFilteredList.stream().sorted(new TopicNameComparator()).collect(Collectors.toList());

        return getConnectorModelsList(topicsFromSOT, pageNo, currentPage, listAllEnvs, orderOfEnvs, tenantId);
    }

    private List<KafkaConnectorModel> getConnectorModelsList(List<KwKafkaConnector> topicsFromSOT, String pageNo, String currentPage,
    ...
    private List<KwKafkaConnector> groupConnectorsByEnv(List<KwKafkaConnector> topicsFromSOT) {
        ...
        Map<String, List<KwKafkaConnector>> groupedList = topicsFromSOT.stream()
                .collect(Collectors.groupingBy(KwKafkaConnector::getConnectorName));
    ...
    public String deleteConnectorRequests(String topicId) {
        ...
        String deleteTopicReqStatus = manageDatabase.getHandleDbRequests().deleteConnectorRequest(Integer.parseInt(topicId),
                commonUtilsService.getTenantId(getUserName()));

        return "{\"result\":\""+deleteTopicReqStatus+"\"}";
    }

    public List<KafkaConnectorRequestModel> getCreatedConnectorRequests(String pageNo, String currentPage, String requestsType) {
        ...
        if(commonUtilsService.isNotAuthorizedUser(getPrincipal(), PermissionType.APPROVE_ALL_REQUESTS_TEAMS))
            createdTopicReqList = manageDatabase.getHandleDbRequests()
    ...
    private List<KafkaConnectorRequestModel> updateCreateConnectorReqsList(List<KafkaConnectorRequest> topicsList, int tenantId){
        List<KafkaConnectorRequestModel> topicRequestModelList = getConnectorRequestModels(topicsList, true);
    ...
    public HashMap<String, String> approveConnectorRequests(String connectorId) throws KafkawizeException {
        ...
        KafkaConnectorRequest connectorRequest = manageDatabase.getHandleDbRequests()
        ...
        if(connectorRequest.getConnectortype().equals(TopicRequestTypes.Claim.name())){
            List<KwKafkaConnector> allTopics = getConnectorsFromName(connectorRequest.getConnectorName(), tenantId);
            for (KwKafkaConnector allTopic : allTopics) {
                allTopic.setTeamId(connectorRequest.getTeamId()); // for claim reqs, team stored in description
            ...
            if(Objects.equals(updateTopicReqStatus, "success")) {
                ...
                mailService.sendMail(connectorRequest.getConnectorName(), null, "", connectorRequest.getRequestor(),
                        dbHandle, CONNECTOR_REQUEST_APPROVED, commonUtilsService.getLoginUrl());
            }
        }
    ...
    private void setConnectorHistory(KafkaConnectorRequest connectorRequest, String userName, int tenantId){
        try {
            ObjectMapper objectMapper = new ObjectMapper();
            ...
            if(connectorRequest.getConnectortype().equals(TopicRequestTypes.Update.name())){
                List<KwKafkaConnector> existingTopicList =  getConnectorsFromName(connectorRequest.getConnectorName(), tenantId);
            ...
            topicHistory.setTeamName(manageDatabase.getTeamNameFromTeamId(tenantId, connectorRequest.getTeamId()));
            topicHistory.setEnvironmentName(getKafkaConnectEnvDetails(connectorRequest.getEnvironment()).getName());
    ...
    public String declineConnectorRequests(String connectorId, String reasonForDecline) {
        ...
        HandleDbRequests dbHandle = manageDatabase.getHandleDbRequests();
        KafkaConnectorRequest connectorRequest = dbHandle.selectConnectorRequestsForConnector(Integer.parseInt(connectorId), tenantId);
        ...
        mailService.sendMail(connectorRequest.getConnectorName(), null, reasonForDecline,
                connectorRequest.getRequestor(), dbHandle, CONNECTOR_REQUEST_DENIED, commonUtilsService.getLoginUrl());
    ...
    public HashMap<String, String> createConnectorDeleteRequest(String connectorName, String envId) {
        ...
        if(commonUtilsService.isNotAuthorizedUser(getPrincipal(), PermissionType.REQUEST_DELETE_CONNECTORS)){
        ...
        if(topics!=null && topics.size()>0
                && !topics.get(0).getTeamId().equals(userTeamId))
        {
            hashMap.put("result","Failure. Sorry, you cannot delete this connector, as you are not part of this team.");
            return hashMap;
        }
        ...
        Optional<KwKafkaConnector> topicOb = getConnectorsFromName(connectorName, tenantId).stream()
                .filter(topic -> topic.getEnvironment().equals(topicRequestReq.getEnvironment()))
                .findFirst();

        if(manageDatabase.getHandleDbRequests().selectConnectorRequests(topicRequestReq.getConnectorName(),
                topicRequestReq.getEnvironment(), RequestStatus.created.name(), tenantId).size()>0){
        ...
        if(topicOb.isPresent()) {
            ...
            mailService.sendMail(topicRequestReq.getConnectorName(), null, "", userDetails,
                    dbHandle, CONNECTOR_DELETE_REQUESTED, commonUtilsService.getLoginUrl());

            hashMap.put("result",manageDatabase.getHandleDbRequests().requestForConnector(topicRequestReq).get("result"));
        }else {
    ...
    public List<KafkaConnectorRequestModel> getConnectorRequests(String pageNo, String currentPage, String requestsType) {
        log.debug("getConnectorRequests page {} requestsType {}", pageNo, requestsType);
        String userDetails = getUserName();
        int tenantId = commonUtilsService.getTenantId(getUserName());
        List<KafkaConnectorRequest> topicReqs = manageDatabase.getHandleDbRequests().getAllConnectorRequests(userDetails, tenantId);
        ...
        topicReqs = topicReqs.stream()
                ...
                .sorted(Collections.reverseOrder(Comparator.comparing(KafkaConnectorRequest::getRequesttime)))
                .collect(Collectors.toList());
    ...
    public HashMap<String, String> createClaimConnectorRequest(String connectorName, String envId) {
        ...
        if(manageDatabase.getHandleDbRequests().selectConnectorRequests(connectorName,
                envId, RequestStatus.created.name(), tenantId).size()>0){
        ...
        Optional<UserInfo> topicOwnerContact = manageDatabase.getHandleDbRequests().selectAllUsersInfo(tenantId).stream()
        ...
        mailService.sendMail(connectorRequest.getConnectorName(), null, "", userDetails,
                dbHandle, CONNECTOR_CLAIM_REQUESTED, commonUtilsService.getLoginUrl());

        topicOwnerContact.ifPresent(userInfo -> mailService.sendMail(connectorRequest.getConnectorName(), null, "",
                userInfo.getUsername(), dbHandle, CONNECTOR_CLAIM_REQUESTED, commonUtilsService.getLoginUrl()));

        resultMap.put("result",manageDatabase.getHandleDbRequests().requestForConnector(connectorRequest).get("result"));
    ...
    public ConnectorOverview getConnectorOverview(String connectorNamesearch) {
        ...
        List<KwKafkaConnector> topicsSearchList = manageDatabase.getHandleDbRequests()
                .getConnectorsFromName(connectorNamesearch, tenantId);
        ...
        for (KafkaConnectorModel topicInfo : topicInfoList) {
            ...
            if (topicOwnerTeam.equals(loggedInUserTeam) && reqTopicsEnvsList.contains(topicInfo.getEnvironmentId())) {
        ...
        try {
            if (topicOwnerTeam.equals(loggedInUserTeam)) {
                topicOverview.setPromotionDetails(getConnectorPromotionEnv(connectorNamesearch, connectors, tenantId));
    ...
    public HashMap<String, Object> getConnectorDetailsPerEnv(String envId, String connectorName) {
        ...
        List<KwKafkaConnector> connectors = manageDatabase.getHandleDbRequests().getConnectors(connectorName, tenantId);
        ...
        if(connectors.size() == 0)
        {
            ...
            Optional<KwKafkaConnector> topicDescFound = connectors.stream()
                    .filter(topic -> topic.getDescription()!=null && topic.getDescription().length()>0)
                    .findFirst();
            if(topicDescFound.isPresent()){
                topicDescription = topicDescFound.get().getDescription();
            }
            connectors = connectors.stream().filter(topic -> topic.getEnvironment().equals(envId)).collect(Collectors.toList());
        }

        for (KwKafkaConnector kafkaConnector : connectors) {
            connectorModel.setEnvironmentName(getKafkaConnectEnvDetails(kafkaConnector.getEnvironment()).getName());
            ...
            connectorModel.setTeamName(manageDatabase.getTeamNameFromTeamId(tenantId, kafkaConnector.getTeamId()));
            hashMap.put("connectorId","" + kafkaConnector.getConnectorId());
        }

        connectorModel.setDescription(topicDescription);

        String loggedInUserTeam = manageDatabase.getHandleDbRequests()
                .selectAllTeamsOfUsers(getUserName(), tenantId).get(0).getTeamname();
    ...
    private HashMap<String, String> getConnectorPromotionEnv(String topicSearch, List<KwKafkaConnector> kafkaConnectors, int tenantId)
    {
        HashMap<String, String> hashMap = new HashMap<>();
        try {
            ...
            if (kafkaConnectors != null && kafkaConnectors.size() > 0) {
                ...
                kafkaConnectors.stream()
                        ...
                        .findFirst().ifPresent(a -> sourceConnectorConfig.set(a.getConnectorConfig()));
    ...
    public HashMap<String, String> saveConnectorDocumentation(KafkaConnectorModel topicInfo) {
        ...
        HandleDbRequests handleDb = manageDatabase.getHandleDbRequests();
        List<KwKafkaConnector> topicsSearchList = manageDatabase.getHandleDbRequests()
                .getConnectorsFromName(topicInfo.getConnectorName(), commonUtilsService.getTenantId(getUserName()));
        ...
        if (topicOwnerTeam.equals(loggedInUserTeam)) {
            saveResult.put("result", manageDatabase.getHandleDbRequests().updateConnectorDocumentation(topic));
        }
        else
            saveResult.put("result", "failure");

        return saveResult;
    }


    private List<KafkaConnectorRequestModel> getConnectorRequestModels(List<KafkaConnectorRequest> topicsList, boolean fromSyncTopics) {
        ...
        for (KafkaConnectorRequest topicReq : topicsList) {
            ...
            topicRequestModel.setTeamName(manageDatabase.getTeamNameFromTeamId(tenantId, topicRequestModel.getTeamId()));
            ...
            if(fromSyncTopics) {
                ...
                if (!topicRequestModel.getConnectorStatus().equals(RequestStatus.approved.name())) {
                    if (topicRequestModel.getConnectortype() !=null &&
                        ...
                        List<KwKafkaConnector> topics = getConnectorsFromName(topicRequestModel.getConnectorName(), tenantId);
                        topicRequestModel.setApprovingTeamDetails(updateApproverInfo(manageDatabase
                                ...
                                manageDatabase.getTeamNameFromTeamId(tenantId, topics.get(0).getTeamId()), approverRoles, topicRequestModel.getRequestor()));
                    } else
                        topicRequestModel.setApprovingTeamDetails(updateApproverInfo(userList, manageDatabase.getTeamNameFromTeamId(tenantId, userTeamId),
    ...
    private List<KafkaConnectorRequest> getConnectorRequestsPaged(List<KafkaConnectorRequest> origActivityList, String pageNo, String currentPage){
    ...
    public List<KwKafkaConnector> getConnectorsFromName(String connectorName, int tenantId) {
        List<KwKafkaConnector> connectors = manageDatabase.getHandleDbRequests().getConnectorsFromName(connectorName, tenantId);
    ...
    private List<KafkaConnectorRequest> getConnectorRequestsFilteredForTenant(List<KafkaConnectorRequest> createdTopicReqList) {
        // tenant filtering
        try {
            List<String> allowedEnvIdList = getEnvsFromUserId(getUserName());
    ...
    private List<KwKafkaConnector> getFilteredConnectorsForTenant(List<KwKafkaConnector> connectorsFromSOT) {
        // tenant filtering
        try {
            List<String> allowedEnvIdList = getEnvsFromUserId(getUserName());
    ...
    public Env getKafkaConnectEnvDetails(String envId){
        Optional<Env> envFound = manageDatabase.getKafkaConnectEnvList(commonUtilsService.getTenantId(getUserName()))
