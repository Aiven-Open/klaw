...
public class UtilControllerService {
  ...
  public HashMap<String, String> getAuth() {
    ...
    if (userName != null) {
      ...
      else dashboardData.put("supportlink", "https://github.com/muralibasani/kafkawize/issues");

      // broadcast text
      String broadCastText = "";
      String broadCastTextLocal =
          manageDatabase.getKwPropertyValue(broadCastTextProperty, tenantId);
      String broadCastTextGlobal =
          manageDatabase.getKwPropertyValue(broadCastTextProperty, DEFAULT_TENANT_ID);
  ...
  public void shutdownContext() {
    ...
    if (!commonUtilsService.isNotAuthorizedUser(
        getPrincipal(), PermissionType.SHUTDOWN_KAFKAWIZE)) {
