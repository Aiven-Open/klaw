...
public class ManageDatabase implements ApplicationContextAware {
  ...
  private void loadStaticDataToDb() {
    ...
    if (userExists == null) {
      handleDbRequests.addNewUser(
          defaultDataService.getUser(
              ...
              "SUPERADMIN",
              handleDbRequests.selectTeamDetailsFromName(infraTeam, DEFAULT_TENANT_ID).getTeamId(),
              kwAdminMailId,
              userName,
              encryptorSecretKey));
    }
    ...
    List<KwRolesPermissions> kwRolesPerms =
        handleDbRequests.getRolesPermissionsPerTenant(DEFAULT_TENANT_ID);
    if (kwProps == null || kwProps.isEmpty()) {
      handleDbRequests.insertDefaultKwProperties(
          defaultDataService.createDefaultProperties(DEFAULT_TENANT_ID, kwAdminMailId));
    }
    ...
    if (kwRolesPerms == null || kwRolesPerms.isEmpty()) {
      handleDbRequests.insertDefaultRolesPermissions(
          defaultDataService.createDefaultRolesPermissions(
              DEFAULT_TENANT_ID, true, kwInstallationType));
    }
