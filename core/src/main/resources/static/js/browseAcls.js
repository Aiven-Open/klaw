...
app.controller("browseAclsCtrl", function($scope, $http, $location, $window) {
    ...
    $scope.getSchemaOfTopic = function(){
        ...
        if($scope.schemaDetails != null && $scope.schemaDetails != undefined && $scope.schemaDetails.length > 0 && $scope.newSchemaVersion === $scope.displayedSchemaVersion)
            return;
        ...
        $scope.ShowSpinnerStatusSchemas = true;
            $http({
                ...
                if(output.schemaDetails && output.schemaDetails != undefined && output.schemaDetails.length > 0 ) {
                    $scope.schemaDetails = output.schemaDetails;
                    $scope.schemaExists = output.schemaExists;
