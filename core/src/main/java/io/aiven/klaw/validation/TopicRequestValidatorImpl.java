...
public class TopicRequestValidatorImpl
  ...
  public boolean isValid(
    ...
    if (topicRequestModel.getTopicname() == null
        || topicRequestModel.getTopicname().length() == 0) {
    ...
    if (topics != null
        ...
        && !Objects.equals(
            ...
            manageDatabase
                .getHandleDbRequests()
                .getUsersInfo(topicControllerService.getUserName())
                .getTeamId())) {
      updateConstraint(
  ...
  private boolean checkIfPromotionOfTopic(
    ...
    boolean promotionOrderCheck =
        checkInPromotionOrder(topicRequestModel.getEnvironment(), orderOfEnvs);
      ...
      if (promotionOrderCheck) {
        ...
        if (devTopicFound != 1) {
          if (topicControllerService.getEnvDetails(syncCluster) == null) {
            updateConstraint(
                constraintValidatorContext, "Failure. This topic does not exist in base cluster.");
          } else {
            updateConstraint(
                constraintValidatorContext,
