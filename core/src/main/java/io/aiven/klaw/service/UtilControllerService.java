...
public class UtilControllerService implements InitializingBean {
  ...
  public ApiResponse resetCache(ResetEntityCache resetEntityCache, String authorizationHeader) {
    String entityType = resetEntityCache.getEntityType();
    String operationType = resetEntityCache.getOperationType();

    if (EntityType.TENANT.name().equals(entityType) && "DELETE".equalsIgnoreCase(operationType)) {
      log.warn("Blocked attempt to delete tenant via resetCache endpoint");
      return ApiResponse.notOk("Tenant deletion is not allowed via this endpoint");
    }
    ...
    // Check for valid internal service token (server-to-server communication)
    boolean isInternalServiceCall =
        commonUtilsService.isValidInternalServiceToken(authorizationHeader);

    if (isInternalServiceCall) {
      log.info("Reset cache triggered by internal service on instance {}", resetEntityCache);
    } else {
      // For regular user requests, check authentication and authorization
      if (getUserName().equals(ANONYMOUS_USER)) {
        log.warn("Unauthorized attempt to reset cache by anonymous user");
        return ApiResponse.NOT_AUTHORIZED;
      }
      // Check for UPDATE_SERVERCONFIG permission for non-internal calls
      if (commonUtilsService.isNotAuthorizedUser(
          commonUtilsService.getPrincipal(), PermissionType.UPDATE_SERVERCONFIG)) {
        log.warn(
            "Unauthorized attempt to reset cache by user without permission: {}", getUserName());
        return ApiResponse.NOT_AUTHORIZED;
      }
      if (entityType.equals(EntityType.USERS.name())
          && commonUtilsService.isNotAuthorizedUser(
              commonUtilsService.getPrincipal(), PermissionType.ADD_EDIT_DELETE_USERS)) {
        return ApiResponse.NOT_AUTHORIZED;
      }
